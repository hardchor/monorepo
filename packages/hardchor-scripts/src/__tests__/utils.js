...
test('resolveKcdScripts resolves to src/index.js when in the hardchor-scripts package', () => {
...
test('resolveKcdScripts resolves to hardchor-scripts if not in the hardchor-scripts package', () => {
  mockPkg({pkg: {name: 'not-hardchor-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveKcdScripts()).toBe('hardchor-scripts')
})
